    ...
    <div class="max-w-7xl mx-auto px-6 py-12 flex flex-col gap-16">
      ...
      <section>
        ...
        <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          <RouterLink v-for="artist in (artistsData as any)?.topartists?.artist" :key="artist.name"
      ...
      <section>
        ...
        <div v-if="loadingAlbums" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          <SkeletonCard v-for="n in 12" :key="n" />
        </div>
        <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          <RouterLink v-for="album in (albumsData as any)?.albums?.album" :key="album.name"
            ...
            <div
              ...
              <img v-if="album.image?.find((i: any) => i.size === 'extralarge')?.['#text']"
                :src="album.image.find((i: any) => i.size === 'extralarge')['#text']" :alt="album.name"
